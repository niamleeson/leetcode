...
const GRADE_LABELS: Record<StudyGrade, { label: string; className: string }> = {
  0: { label: 'Again', className: 'text-red-300 border-red-900/50 bg-red-950/30' },
  1: { label: 'Hard', className: 'text-orange-300 border-orange-900/50 bg-orange-950/30' },
  2: { label: 'Good', className: 'text-blue-300 border-blue-900/50 bg-blue-950/30' },
  3: { label: 'Easy', className: 'text-emerald-300 border-emerald-900/50 bg-emerald-950/30' },
};
...
export default function LlmGradeCard({ request, onApply, onRetry }: Props) {
  ...
  if (state === 'loading') {
    return (
      <div className="border border-indigo-900/40 bg-indigo-950/20 rounded-md p-3">
  ...
  if (state === 'error') {
    return (
      <div className="border border-amber-900/40 bg-amber-950/20 rounded-md p-3 space-y-2">
        <div className="text-xs text-amber-300">Grader unavailable — advancing as Good.</div>
        <button
          onClick={() => onApply(2)}
  ...
  return (
    <div className="border border-indigo-900/40 bg-indigo-950/20 rounded-md p-4 space-y-3">
      ...
      {result.rationale && (
        <div className="text-xs text-gray-300 italic">"{result.rationale}"</div>
      )}
      ...
      <div className="flex gap-2">
        {onRetry && (
          <button
            onClick={onRetry}
            className="bg-gray-800 hover:bg-gray-700 text-gray-200 text-sm px-4 py-2 rounded-md flex-1"
          >
            Try again
          </button>
        )}
...
function ShowAnswerToggle({ canonical }: { canonical: string }) {
  ...
  return (
    <div>
      ...
      {show && (
        <div className="mt-2 border border-gray-800 bg-gray-900/60 rounded-md p-3 max-h-60 overflow-y-auto">
