...
function MermaidBlock({ chart }: { chart: string }) {
  ...
  if (!svgHtml) return <div className="text-gray-600 text-xs p-4">Rendering diagram...</div>;
  return (
    <div className="my-3 p-2 bg-[#11111b] border border-gray-800 rounded-lg overflow-x-auto flex justify-center" dangerouslySetInnerHTML={{ __html: svgHtml }} />
  );
}
            ...
            if (isBlock) {
              return (
                <pre className="!bg-gray-950 !border !border-gray-800 !rounded-md !p-4 !text-xs overflow-x-auto !leading-relaxed !m-0 mb-3">
                  <code className={lang ? `language-${lang}` : 'text-gray-300 font-mono whitespace-pre'}>
                    {text.trim()}
                  </code>
                </pre>
              );
