...
import { neetcodeTopics, NeetCodeTopic, NeetCodeTemplate, NeetCodeProblem } from '../data/neetcode250';
...
export default function NeetCode250() {
  const [language, setLanguage] = useLocalStorage<Lang>('nc250-language', 'javascript');
  const inAppIds = useMemo(() => new Set(uniqueProblems.map((p) => p.id)), []);
  const topicsOpen = usePersistedOpenSet<string>('nc250-open-topics');
  const problemsOpen = usePersistedOpenSet<number>('nc250-open-problems');

  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-white">NeetCode 250</h1>
          <p className="text-gray-400 mt-1 text-sm">
            Topic &rarr; template &rarr; questions. Memorize the pseudocode, then see it solve many problems.
          </p>
        </div>
